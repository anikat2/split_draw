...
const API_BASE = "https://split-draw-1.onrender.com";
...
export default function App() {
  ...
  const connectWs = useCallback((lid, uid) => {
    return new Promise((resolve, reject) => {
      ...
      ws.onerror = (e) => {
        ...
        setError("Connection failed — check the lobby code and try again");
  ...
  async function createLobby() {
    ...
    try {
      ...
      await connectWs(data.lobby_id, userId);
      setScreen("lobby");
    } catch (e) {
  ...
  async function joinLobby() {
    ...
    setLobbyId(lid);
    try {
      await connectWs(lid, userId);
      setScreen("lobby");
    } catch {
      // error already set in connectWs
    } finally {
      setConnecting(false);
    }
  }
  ...
  async function startGame() {
    const res = await fetch(`${API_BASE}/begin_round/${lobbyId}`);
    const data = await res.json();
...
function HomeScreen({ onCreate, onJoin, joinInput, setJoinInput, error, connecting }) {
  return (
    <div style={styles.centerPage}>
      <div style={styles.homeCard}>
        ...
        <button style={{...styles.btnPrimary, opacity: connecting ? 0.6 : 1}} onClick={onCreate} disabled={connecting}>
        ...
        <div style={styles.joinRow}>
          <input
            ...
            disabled={connecting}
          />
          <button style={{...styles.btnSecondary, opacity: connecting ? 0.6 : 1}} onClick={onJoin} disabled={connecting}>
            {connecting ? "…" : "join"}
          </button>
        </div>
...
function Round1Screen({ gameState, sendWs }) {
  ...
  function clearCanvas() {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext("2d");
    ctx.fillStyle = "#f9f5f0"; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.strokeStyle = "#ccc"; ctx.setLineDash([6, 4]);
    ctx.beginPath(); ctx.moveTo(canvas.width / 2, 0); ctx.lineTo(canvas.width / 2, canvas.height); ctx.stroke();
    ctx.setLineDash([]);
    ctx.fillStyle = "rgba(0,0,0,0.08)"; ctx.font = "11px monospace"; ctx.textAlign = "center";
    ctx.fillText("← draw your half here", canvas.width / 4, 24);
  }
    ...
    <div style={styles.gamePage}>
      ...
      <div style={styles.toolbar}>
        <div style={styles.paletteRow}>
          {palette.map(c => (
            <button key={c} onClick={() => setColor(c)} style={{...styles.colorSwatch, background: c, outline: color === c ? `3px solid #e9c46a` : "none", outlineOffset: "2px"}} />
          ))}
        </div>
...
function Round2Screen({ gameState, sendWs }) {
  ...
  return (
    <div style={styles.gamePage}>
      ...
      <p style={styles.instruction}>your partner drew some, you draw the rest · AI is also completing it simultaneously</p>
      <div style={styles.canvasWrapper}>
        <canvas ref={canvasRef} width={600} height={400} style={styles.canvas}
          onMouseDown={startDraw} onMouseMove={draw} onMouseUp={stopDraw} onMouseLeave={stopDraw}
          onTouchStart={startDraw} onTouchMove={draw} onTouchEnd={stopDraw} />
...
function Round3Screen({ gameState, sendWs, userId }) {
  ...
  }, [voted, sendWs, gameState]);

  useEffect(() => {
    if (voted) return;
    const t = setInterval(() => {
      setTimeLeft(p => {
        if (p <= 1) { clearInterval(t); handleAutoVote(); return 0; }
        return p - 1;
      });
    }, 1000);
    return () => clearInterval(t);
  }, [voted, handleAutoVote]);
...
function ResultsScreen({ gameState }) {
  ...
  return (
    <div style={styles.gamePage}>
      ...
      {results.map((r, i) => (
        <div key={r.target} style={styles.resultCard}>
          <div style={styles.resultHeader}>
            drawing {i + 1} ·{" "}
            <span style={{color: r.humans_fooled ? "#e63946" : "#2a9d8f"}}>
              {r.humans_fooled ? "AI fooled you" : "humans won"}
            </span>
            <span style={styles.resultVotes}>
              {r.human_votes}v human · {r.ai_votes}v AI
            </span>
          </div>
          <div style={styles.resultImgRow}>
            <div style={styles.resultImgBlock}>
              <div style={styles.resultImgLabel}>human completion</div>
              {r.human_img
                ? <img src={`data:image/png;base64,${r.human_img}`} alt="human" style={styles.resultImg} />
      ...
      <p style={{...styles.instruction, marginTop: "1rem"}}>refresh to play again</p>
    </div>
  );
}
...
function VoteCard({ label, image, chosen, disabled, onVote }) {
  return (
    <div style={{...styles.voteCard, outline: chosen ? "3px solid #e9c46a" : "3px solid transparent", opacity: disabled && !chosen ? 0.6 : 1, cursor: disabled ? "default" : "pointer"}}
      ...
      <div style={{...styles.voteLabel, background: chosen ? "#e9c46a" : "rgba(0,0,0,0.75)", color: chosen ? "#1a1a1a" : "#fff"}}>
        {chosen ? "✓ " : ""}{label}
      </div>
    </div>
  );
...
const styles = {
  root: { minHeight: "100vh", background: "#0f0e0d", color: "#f9f5f0", fontFamily: "'Courier New', Courier, monospace", position: "relative", overflow: "hidden" },
  ...
  centerPage: { minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", position: "relative", zIndex: 1 },
  homeCard: { background: "#1a1916", border: "1px solid #333", borderRadius: "4px", padding: "2.5rem 2rem", maxWidth: "380px", width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: "0.75rem" },
  logoMark: { fontSize: "28px", color: "#e9c46a", lineHeight: 1 },
  title: { fontSize: "32px", fontWeight: "700", letterSpacing: "0.2em", color: "#f9f5f0", margin: 0, fontFamily: "'Courier New', Courier, monospace" },
  subtitle: { fontSize: "12px", color: "#888", textAlign: "center", lineHeight: 1.6, margin: 0, fontFamily: "'Courier New', Courier, monospace" },
  divider: { width: "100%", height: "1px", background: "#333", margin: "0.5rem 0" },
  btnPrimary: { background: "#e9c46a", color: "#1a1a1a", border: "none", borderRadius: "2px", padding: "10px 24px", fontFamily: "'Courier New', Courier, monospace", fontWeight: "700", fontSize: "13px", letterSpacing: "0.05em", cursor: "pointer", width: "100%", transition: "opacity 0.15s" },
  btnSecondary: { background: "transparent", color: "#f9f5f0", border: "1px solid #555", borderRadius: "2px", padding: "10px 16px", fontFamily: "'Courier New', Courier, monospace", fontSize: "13px", cursor: "pointer", flexShrink: 0 },
  btnTool: { background: "transparent", color: "#aaa", border: "1px solid #444", borderRadius: "2px", padding: "6px 12px", fontFamily: "'Courier New', Courier, monospace", fontSize: "12px", cursor: "pointer" },
  input: { background: "#111", color: "#f9f5f0", border: "1px solid #444", borderRadius: "2px", padding: "10px 12px", fontFamily: "'Courier New', Courier, monospace", fontSize: "16px", letterSpacing: "0.15em", flex: 1, outline: "none" },
  joinRow: { display: "flex", gap: "8px", width: "100%" },
  errorText: { color: "#e63946", fontSize: "12px", margin: 0 },
  labelSmall: { fontSize: "11px", color: "#888", letterSpacing: "0.08em", textTransform: "uppercase", margin: 0, fontFamily: "'Courier New', Courier, monospace" },
  lobbyCode: { fontSize: "42px", fontWeight: "700", letterSpacing: "0.3em", color: "#e9c46a", margin: "0.25rem 0", fontFamily: "'Courier New', Courier, monospace" },
  gamePage: { minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", padding: "1.5rem 1rem", position: "relative", zIndex: 1, gap: "0.75rem" },
  gameHeader: { display: "flex", alignItems: "center", gap: "1rem", width: "100%", maxWidth: "660px", flexWrap: "wrap" },
  roundBadge: { background: "#2a9d8f", color: "#fff", fontSize: "11px", fontWeight: "700", letterSpacing: "0.1em", padding: "4px 10px", borderRadius: "2px", flexShrink: 0, fontFamily: "'Courier New', Courier, monospace" },
  ...
  paletteRow: { display: "flex", gap: "6px", flexWrap: "wrap" },
  colorSwatch: { width: "24px", height: "24px", border: "1px solid #555", borderRadius: "2px", cursor: "pointer", padding: 0, flexShrink: 0 },
  toolRow: { display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" },
  waitBadge: { fontSize: "12px", color: "#e9c46a", fontFamily: "'Courier New', Courier, monospace", padding: "6px 12px", border: "1px solid #e9c46a55", borderRadius: "2px" },
  voteGrid: { display: "flex", gap: "1.5rem", width: "100%", maxWidth: "780px", flexWrap: "wrap", justifyContent: "center" },
  voteCard: { position: "relative", background: "#1a1916", border: "1px solid #333", borderRadius: "4px", overflow: "hidden", cursor: "pointer", transition: "outline 0.1s, opacity 0.2s", flex: "1 1 300px", maxWidth: "360px" },
  voteImg: { width: "100%", height: "auto", display: "block" },
  voteImgPlaceholder: { height: "240px", display: "flex", alignItems: "center", justifyContent: "center", color: "#555", fontSize: "13px", fontFamily: "'Courier New', Courier, monospace" },
  voteLabel: { position: "absolute", bottom: 0, left: 0, right: 0, padding: "8px", textAlign: "center", fontWeight: "700", fontSize: "14px", letterSpacing: "0.15em", fontFamily: "'Courier New', Courier, monospace", transition: "background 0.15s, color 0.15s" },
  votedBanner: { fontSize: "13px", color: "#e9c46a", border: "1px solid #e9c46a44", borderRadius: "2px", padding: "8px 20px", fontFamily: "'Courier New', Courier, monospace", letterSpacing: "0.05em" },
  scoreBanner: { display: "flex", gap: "2rem", justifyContent: "center", padding: "1rem 2rem", background: "#1a1916", border: "1px solid #333", borderRadius: "2px", width: "100%", maxWidth: "660px" },
  scoreBlock: { display: "flex", flexDirection: "column", alignItems: "center", gap: "4px", color: "#2a9d8f" },
  scoreNum: { fontSize: "42px", fontWeight: "700", fontFamily: "'Courier New', Courier, monospace", lineHeight: 1 },
  scoreLabel: { fontSize: "11px", color: "#888", letterSpacing: "0.08em", textTransform: "uppercase", fontFamily: "'Courier New', Courier, monospace" },
  resultCard: { width: "100%", maxWidth: "660px", background: "#1a1916", border: "1px solid #333", borderRadius: "2px", padding: "1rem" },
  resultHeader: { fontSize: "12px", color: "#888", fontFamily: "'Courier New', Courier, monospace", marginBottom: "0.75rem", display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" },
  resultVotes: { marginLeft: "auto", color: "#555", fontSize: "11px", fontFamily: "'Courier New', Courier, monospace" },
  resultImgRow: { display: "flex", gap: "1rem", flexWrap: "wrap" },
  resultImgBlock: { flex: "1 1 220px" },
  resultImgLabel: { fontSize: "10px", color: "#2a9d8f", letterSpacing: "0.08em", textTransform: "uppercase", fontFamily: "'Courier New', Courier, monospace", marginBottom: "4px" },
  resultImg: { width: "100%", height: "auto", display: "block", border: "1px solid #333", borderRadius: "2px" },
};